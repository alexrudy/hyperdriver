...
async fn echo_h1() {
    ...
    let server = platter::Server::new_with_protocol(
        acceptor,
        tower::service_fn(|_| async { Ok::<_, hyper::Error>(tower::service_fn(echo)) }),
        platter::conn::http1::Builder::new(),
    );
...
async fn echo_h2() {
    ...
    let server = platter::Server::new_with_protocol(
        acceptor,
        tower::service_fn(|_| async { Ok::<_, hyper::Error>(tower::service_fn(echo)) }),
        platter::conn::http2::Builder::new(TokioExecutor::new()),
    );
...
async fn echo_auto_h1() {
    ...
    let server = platter::Server::new_with_protocol(
        acceptor,
        tower::service_fn(|_| async { Ok::<_, hyper::Error>(tower::service_fn(echo)) }),
        platter::conn::auto::Builder::new(TokioExecutor::new()),
    );
