...
use hyperdriver_core::info::tls::{channel, TlsConnectionInfoReciever, TlsConnectionInfoSender};
use hyperdriver_core::info::{ConnectionInfo, HasConnectionInfo, TlsConnectionInfo};
use hyperdriver_core::stream::tls::{TlsHandshakeInfo, TlsHandshakeStream};
...
mod tests {
    ...
    use hyperdriver_client::conn::transport::duplex::DuplexTransport;
    use hyperdriver_client::conn::transport::TransportExt as _;
    use hyperdriver_client::conn::Transport as _;

    use crate::conn::AcceptExt as _;
    ...
    async fn tls_client_server() {
        ...
        let service = tower::service_fn(|_: http::Request<hyperdriver_body::Body>| async {
            Ok::<_, Infallible>(Response::new(hyperdriver_body::Body::empty()))
        });

        let (client, incoming) = hyperdriver_core::stream::duplex::pair();

        let acceptor = crate::conn::Acceptor::from(incoming)
        ...
        let server = async move {
            ...
            let _ = tower::Service::call(
                &mut svc,
                http::Request::new(hyperdriver_body::Body::empty()),
            )
            .await
            .unwrap();
