...
use super::Stream;
#[cfg(feature = "tls")]
use crate::conn::tls::TlsAcceptor as RawTlsAcceptor;
use hyperdriver_core::info::HasConnectionInfo;
use hyperdriver_core::stream::duplex::DuplexIncoming;
#[cfg(feature = "stream")]
...
impl Accept for TcpListener {
    type Conn = hyperdriver_core::stream::tcp::TcpStream;
    type Error = io::Error;
    ...
    fn poll_accept(
        ...
        TcpListener::poll_accept(self.get_mut(), cx)
            .map(|r| r.map(|(stream, addr)| Self::Conn::server(stream, addr)))
    }
}

impl Accept for UnixListener {
    type Conn = hyperdriver_core::stream::unix::UnixStream;
    type Error = io::Error;
    ...
    fn poll_accept(
        ...
        UnixListener::poll_accept(self.get_mut(), cx).map(|r| {
            r.and_then(|(stream, addr)| Ok(Self::Conn::new(stream, Some(addr.try_into()?))))
        })
    }
}

impl Accept for DuplexIncoming {
    type Conn = hyperdriver_core::stream::duplex::DuplexStream;
    type Error = io::Error;
