...
use crate::{conn::protocol::auto::HttpConnectionBuilder, Client};
use hyperdriver_core::info::HasConnectionInfo;
use hyperdriver_core::service::OptionLayerExt;
use hyperdriver_core::service::SharedService;
use hyperdriver_core::service::TimeoutLayer;
...
impl<T, P, RP, S> Builder<T, P, RP, S> {
    ...
    pub fn pool(&mut self) -> Option<&mut crate::pool::Config> {
        self.pool.as_mut()
    }

    /// Use the provided connection pool configuration.
    pub fn with_pool(mut self, pool: crate::pool::Config) -> Self {
        self.pool = Some(pool);
        self
    }
    ...
    pub fn build_service(
        ...
        let service = self
            ...
            .optional(self.retries.map(|attempts| {
                tower::retry::RetryLayer::new(hyperdriver_core::service::Attempts::new(attempts))
            }))
            .optional(
                self.timeout
