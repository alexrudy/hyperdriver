...
use crate::conn::Stream as ClientStream;
...
pub(in crate::conn::transport) mod future {
    use std::fmt;
...
mod tests {
    ...
    //     let (client, server) = hyperdriver_core::stream::duplex::pair();

    //     let mut config = fixtures::tls_client_config();
    //     config.alpn_protocols.push(b"h2".to_vec());
    //     let transport = crate::conn::transport::TlsTransportWrapper::new(
    //         crate::conn::transport::duplex::DuplexTransport::new(1024, client),
    //         config.into(),
    //     );
