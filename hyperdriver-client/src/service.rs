...
where
    ...
    T: Transport + 'static,
    T::IO: Unpin,
    BIn: Into<hyperdriver_body::Body>,
    BOut: From<hyperdriver_body::Body> + Unpin,
    <<T as Transport>::IO as HasConnectionInfo>::Addr: Send,
    ...
    C::ResBody: Into<crate::Body>,
{
    ...
    pub fn request(
        &self,
        request: hyperdriver_body::Request,
...
where
    ...
    T: pool::PoolableTransport,
{
    fn new(checkout: Checkout<C, T, ConnectionError>, request: hyperdriver_body::Request) -> Self {
        Self {
...
where
    ...
    C::ResBody: Into<hyperdriver_body::Body>,
    T: pool::PoolableTransport,
    BOut: From<hyperdriver_body::Body> + Unpin,
{
...
enum ResponseFutureState<C: pool::PoolableConnection, T: pool::PoolableTransport> {
    ...
    Request(BoxFuture<'static, Result<http::Response<hyperdriver_body::Body>, Error>>),
}

/// Prepare a request for sending over the connection.
fn prepare_request<C: Connection + PoolableConnection>(
    request: &mut http::Request<hyperdriver_body::Body>,
    conn: &Pooled<C>,
) -> Result<(), Error> {
    request
        .headers_mut()
...
async fn execute_request<C>(
    mut request: hyperdriver_body::Request,
    mut conn: Pooled<C>,
) -> Result<http::Response<hyperdriver_body::Body>, Error>
where
    C: Connection + PoolableConnection,
    C::ResBody: Into<crate::Body>,
{
...
mod tests {
    ...
    #[cfg(feature = "mocks")]
    use crate::conn::protocol::mock::MockProtocol;
    #[cfg(feature = "mocks")]
    use crate::conn::transport::mock::{MockConnectionError, MockTransport};
