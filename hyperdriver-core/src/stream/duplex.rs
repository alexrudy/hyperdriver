...
//! together, like so:
//! ```
//! # use hyperdriver_core::stream::duplex::{self, DuplexClient};
//! # use std::future::poll_fn;
//! # use std::pin::pin;
//! # async fn demo_duplex() {
//! let (client, incoming) = duplex::pair();
//! let mut server = pin!(incoming);
//! let (client_conn, server_conn) = tokio::try_join!(client.connect(1024), poll_fn(|cx| server.as_mut().poll_incoming(cx))).unwrap();
//!
//! # }
//! ```
...
impl DuplexIncoming {
    ...
    pub fn poll_incoming(
        ...
        cx: &mut Context<'_>,
    ) -> Poll<Result<DuplexStream, io::Error>> {
        if let Some(request) = ready!(self.receiver.poll_recv(cx)) {
            Poll::Ready(Ok(request.ack(self.max_buf_size)?))
        } else {
...
impl futures_core::Stream for DuplexIncoming {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        match ready!(self.poll_incoming(cx)) {
            Ok(stream) => Poll::Ready(Some(Ok(stream))),
            Err(e) if e.kind() == io::ErrorKind::ConnectionReset => Poll::Ready(None),
