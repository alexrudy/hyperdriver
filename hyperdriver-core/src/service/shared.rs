...
mod tests {
    ...
    #[allow(unused)]
    type Body = http_body_util::Empty<Vec<u8>>;

    assert_impl_all!(SharedService<http::Request<Body>, http::Response<Body>, Box<dyn std::error::Error + Send + Sync + 'static>>: Clone, Send, Sync);
}
