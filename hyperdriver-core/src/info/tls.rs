...
#[cfg(feature = "server")]
pub use self::channel::{channel, TlsConnectionInfoReciever, TlsConnectionInfoSender};
...
impl TlsConnectionInfo {
    ...
    #[allow(dead_code)]
    pub fn new_client(alpn: Option<Protocol>) -> Self {
        Self {
            server_name: None,
            validated_server_name: false,
            alpn,
    ...
    /// Mark that the server name has been validated
    /// and that the request and SNI agree.
...
mod channel {
    ...
    impl TlsConnectionInfoReciever {
        ...
        pub fn new(inner: tokio::sync::oneshot::Receiver<TlsConnectionInfo>) -> Self {
            Self {
