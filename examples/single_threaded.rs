    ...
    let client = tower::ServiceBuilder::new()
        .layer(
            ConnectionPoolLayer::new(
                ...
                auto::HttpConnectionBuilder::<Body, _>::new(
                    http1::Builder::new(),
        ...
        .service(RequestExecutor::<HttpConnection<Body>, _, LocalExec>::new(
            LocalExec,
        ));
...
impl Transport for TransportNotSend {
    ...
    fn connect(&mut self, uri: http::Uri) -> <Self as Transport>::Future {
        self.tcp
            .connect(uri)
            .map(|res| res.map(IOTypeNotSend::new))
            .boxed()
    }
